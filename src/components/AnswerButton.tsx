...
const AnswerButton = ({ label, text, pressed, answer, onPress, key, onKeyDown = null, ref = null }: any) => {
    return (
        <button className={`flex border mt-4 first:mt-0 py-2 px-3 rounded shadow hover:shadow-md transition-all cursor-pointer w-full items-center ${pressed ? 'bg-zinc-200 border-2' : ''} ${answer ? 'border-emerald-500 border-2' : ''} ${pressed && !answer ? 'border-zinc-400' : ''}`} role="button" aria-pressed={`${pressed ? 'true' : 'false'}`} onClick={() =>onPress(label) } onKeyDown={ onKeyDown } ref={ ref }>
